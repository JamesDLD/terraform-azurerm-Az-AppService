...
variable "app_services" {
  default = {
    ...
    drupal_sample = {
      id                   = "1"            #(Required)
      prefix               = "dockerdrupal" #(Required) Specifies the name of the App Service. Changing this forces a new resource to be created.
      ...
      http_logs_azure_blob_storage = [
        {
          level             = "Verbose" #(Optional) The level at which to log. Possible values include Error, Warning, Information, Verbose and Off. NOTE: this field is not available for http_logs
          sas_url           = "urlrlrl" #(Optional) The URL to the storage container, with a Service SAS token appended. NOTE: there is currently no means of generating Service SAS tokens with the azurerm provider.
          retention_in_days = "1"       #(Optional) The number of days to retain logs for.
        },
      ]
      */
      site_config = [
